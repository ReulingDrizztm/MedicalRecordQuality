...
class ClientInterface(object):
    ...
    def segmentRecord_new(self, json_file):
            ...
            for collection in his_request:
                ...
                if collection in his_record:
                    if collection == 'yizhu':
                        ...
                        socket_result = socket_app.process(patient_id, '', collection,
                                                           start_time=binganshouye['pat_visit']['admission_time'])
                    ...
                    if collection == 'jianchabaogao':
                        ...
                        self.mongo_push_utils.pushData('mrq_jianchabaogao', {
                            collection: {'exam_report': socket_result.get(collection, list())},
                            '_id': _id,
                        ...
                        self.mongo_push_utils.pushData('mrq_jianyanbaogao', {
                            collection: {'lab_report': socket_result.get(collection, list())},
                            '_id': _id,
                            'patient_id': patient_id,
                            'visit_id': visit_id,
                            'batchno': datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')})
                        ...
                        self.mongo_push_utils.pushData('mrq_jianyanbaogao',
                                                       {collection: socket_result.get(collection, list()),
                                                        '_id': _id,
                                                        'patient_id': patient_id,
                                                        'visit_id': visit_id,
                                                        'batchno': datetime.strftime(datetime.now(),
                                                                                     '%Y-%m-%d %H:%M:%S')})
        ...
        if 'huanzheshouye' in json_file:
            ...
            self.mongo_push_utils.pushData('mrq_binganshouye', {'binganshouye': json_file.get('huanzheshouye', dict()),
                                                                ...
                                                                'batchno': datetime.strftime(datetime.now(),
                                                                                             '%Y-%m-%d %H:%M:%S')})
            self.mongo_push_utils.pushData('mrq_shouyeshoushu',
                                           {'shouyeshoushu': json_file.get('shouyeshoushu', list()),
                                            '_id': _id,
                                            'patient_id': patient_id,
                                            'visit_id': visit_id,
                                            'batchno': datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')})
            self.mongo_push_utils.pushData('mrq_shouyezhenduan',
                                           {'shouyezhenduan': json_file.get('shouyezhenduan', list()),
                                            '_id': _id,
                                            'patient_id': patient_id,
                                            'visit_id': visit_id,
                                            'batchno': datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')})
    ...
    def processJsonFile(self, json_file):
        ...
        json_file为传过来的原始数据
        pageSource: 页面来源
        页面来源   0 :保存病历 1：下诊断 2：打开病例 3：新建病例 6：下医嘱 7：病案首页 8：其他   （没有 4 和 5）
        :param json_file: 传过来的原始数据
        :return:
        """
        if json_file.get('pageSource') == '1':  # pageSource: 页面来源
        ...
        if json_file.get('huanzheshouye'):  # huanzheshouye: 患者首页
        ...
        # 先看连的是否是测试mongo库，如果是测试库，直接进行推送，否则判定文书是什么状态(新建或保存)，再判断账号为非测试账号
        push_flag = False
    ...
    def doctorControlStats(self, ward_name='', record_name='', regular_name='', in_hospital=True, step='医生端'):
        ...
        sheet_info = [{'code': k,
                       ...
                       'value2': modify_tmp.get(k, 0)} for k, v in res_tmp.items() if (
                                  regular_name and self.regular_model[k].get('regular_classify',
                                                                             '') in regular_name) or not regular_name]
        ...
        if in_hospital:
            ...
            for data in doc_total:
                if ward_name == '全部' or ward_name == '医务处' or data[
                    'DEPT_DISCHARGE_FROM_NAME'] == ward_name or self.ward_dept.get(
                        data['DEPT_DISCHARGE_FROM_NAME']) == ward_name:
                    result.setdefault(data['DOCTOR_IN_CHARGE_ID'], dict())
                    # {"医生ID"：{"doctor_name"："名称"，"ward"："病区"，"pat_num"："病人数"}}
    ...
    def doctorRank(self, doctor_id='', dept=''):
        ...
        last_month = datetime.strftime(datetime(year_left, month_left, 1), '%Y-%m')  # 上月
        last_two_month = datetime.strftime(datetime(year_left_left, month_left_left, 1), '%Y-%m')  # 上上月
    ...
    def runDemo(self, json_file):
        ...
        ruyuanjilu = [
            ...
            {'key': 'ryjl_tgjc', 'value': json_file.get('physical_examination', ''),
             'chapter_name': 'physical_examination'},
            {'key': 'ryjl_xbs', 'value': json_file.get('history_of_present_illness', ''),
             'chapter_name': 'history_of_present_illness'},
            {'key': 'ryjl_jws', 'value': json_file.get('history_of_past_illness', ''),
             'chapter_name': 'history_of_past_illness'},
            {'key': 'ryjl_grs', 'value': json_file.get('social_history', ''), 'chapter_name': 'social_history'},
            {'key': 'ryjl_yjhys', 'value': json_file.get('menstrual_and_obstetrical_histories', ''),
             'chapter_name': 'menstrual_and_obstetrical_histories'},
            {'key': 'ryjl_jzs', 'value': json_file.get('history_of_family_member_diseases', ''),
             'chapter_name': 'history_of_family_member_diseases'},
            {'key': 'ryjl_cbzd', 'value': json_file.get('special_examination', ''),
             'chapter_name': 'special_examination'}
        ]
